...
public class Simplex {
    ...
    // custom wrapper for the results
    public class WrapperResults {
        private ERROR error;
        private int PivotColumn;

        public WrapperResults(ERROR error, int PivotColumn) {
            this.error = error;
    ...
    public WrapperResults compute(){
        // step 1
        if(checkOptimality()){
            return new WrapperResults(ERROR.IS_OPTIMAL, 0);
        }
        ...
        if(solutionIsUnbounded)
            return new WrapperResults(ERROR.UNBOUNDED, pivotColumn);
        ...
        return new WrapperResults(ERROR.NOT_OPTIMAL, pivotColumn);
    }
